...
class AssembleModule(CoreModule):
    ...
    def execute(self, config=None, indir=None, outdir=None):
        ...
        if not os.path.isfile(rupturefile):
            ...
            rupturefiles = sorted(
                glob.glob(os.path.join(datadir, "*_fault.txt"))
            )
            rupturefile = None
            if len(rupturefiles):
        ...
        timestamp = datetime.datetime.now(datetime.timezone.utc).strftime("%FT%TZ")
        ...
        self.logger.debug(
            f"Created HDF5 input container in {shake_data.getFileName()}"
        )
        #
        ...
        if len(simfiles) > 1:
            ...
            sim_config = ConfigObj(
                infile=config_file, configspec=f"{sim_config_spec}"
            )
